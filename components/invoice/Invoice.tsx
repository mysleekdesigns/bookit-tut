  ...
  const handleDownload = () => {
    ...
    if (input) {
      ...
      html2canvas(input).then((canvas) => {
        ...
        pdf.save(`invoice_${booking?._id?.toString()}.pdf`);
      });
    }
  };
        ...
        <div className="px-5">
          <div id="booking_invoice" className="px-4 border border-secondary">
            <header className="clearfix">
              ...
              <div id="project">
                <div>
                  <span>Name</span> {(booking?.user as any)?.name}{" "}
                </div>
                <div>
                  <span>EMAIL</span> {(booking?.user as any)?.email}{" "}
                </div>
                <div>
                  <span>DATE</span>{" "}
                ...
                <tbody>
                  <tr>
                    <td className="service">
                      {(booking?.room as any)?.name ?? "N/A"}
                    </td>
                    <td className="desc">
                      {(booking?.room as any)?.pricePerNight ?? "N/A"}
                    </td>
                    <td className="unit">
                    ...
                    <td className="qty">{booking?.daysOfStay?.toString()}</td>
                  </tr>
                  <tr>
