...
const UploadRoomImages = ({ data }: Props) => {
  ...
  const [deleteRoomImage, { error: deleteError, isLoading: isDeleteLoading, isSuccess: isDeleteSuccess }] = useDeleteRoomImageMutation();
  ...
  useEffect(() => {
    if (
      ...
      error.data &&
      "errMessage" in error.data
    ) {
      toast.error(error.data.errMessage as string);
    }

    if (isSuccess) {
      revalidateTag("RoomDetails");
      revalidateTag("RoomList");  // Add this line
      setImagesPreview([]);
      router.refresh();
  ...
  useEffect(() => {
    if (deleteError && "data" in deleteError) {
      toast.error((deleteError.data as { errMessage?: string })?.errMessage || 'An error occurred');
    }
          ...
          >
            ...
            <div className="form-group">
              ...
              <div className="new-images mt-4">
                <p className="text-secondary">New Images:</p>
